TITLE Mod file for component: Component(id=CaDynamics_E2_NML2__decay122__gamma5_09Emin4 type=concentrationModelHayEtAl)

COMMENT

    This NEURON file has been generated by org.neuroml.export (see https://github.com/NeuroML/org.neuroml.export)
         org.neuroml.export  v1.9.0
         org.neuroml.model   v1.9.0
         jLEMS               v0.10.7

ENDCOMMENT

NEURON {
    SUFFIX CaDynamics_E2_NML2__decay122__gamma5_09Emin4
    USEION ca READ cai, cao, ica WRITE cai VALENCE 2
    RANGE cai
    RANGE cao
    GLOBAL initialConcentration
    GLOBAL initialExtConcentration
    RANGE gamma                             : parameter
    RANGE minCai                            : parameter
    RANGE decay                             : parameter
    RANGE depth                             : parameter
    RANGE Faraday                           : parameter
    RANGE currDensCa                        : derived variable
    
}

UNITS {
    
    (nA) = (nanoamp)
    (uA) = (microamp)
    (mA) = (milliamp)
    (A) = (amp)
    (mV) = (millivolt)
    (mS) = (millisiemens)
    (uS) = (microsiemens)
    (molar) = (1/liter)
    (kHz) = (kilohertz)
    (mM) = (millimolar)
    (um) = (micrometer)
    (umol) = (micromole)
    (S) = (siemens)
    
}

PARAMETER {
    gamma = 5.09E-4 
    minCai = 1.0E-4 (mM)
    decay = 122 (ms)
    depth = 0.1 (um)
    Faraday = 0.0964853 (C / umol)
}

ASSIGNED {
    iCa (nA)
    initialConcentration (mM)
    initialExtConcentration (mM)
    currDensCa (nA / um2)                  : derived variable
    rate_concentration (mM/ms)
}

STATE {
    concentration (mM) 
    extConcentration (mM) 
    cai
}

INITIAL {
    initialConcentration = cai
    initialExtConcentration = cao
    concentration = initialConcentration
    extConcentration = initialExtConcentration
}

BREAKPOINT {
    SOLVE states METHOD cnexp
    cai = concentration
}

DERIVATIVE states {
    iCa = -1 * (0.01) * ica :   iCa has units (nA) ; ica (built in to NEURON) has units (mA/cm2)...
    
    currDensCa = iCa ? evaluable
    concentration' =  (  currDensCa   *   gamma  /(2 *  Faraday  *   depth  )) - ((  concentration   -   minCai  ) /   decay  ) ? Note units of all quantities used here need to be consistent!
}
